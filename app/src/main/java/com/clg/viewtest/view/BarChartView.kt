        ...
        for (j in xAxisList.indices) {
            ...
            if (isShowValueText) {
                ...
                canvas.drawText(
                    ...
                    (startY.toDouble() - keduTextSpace.toDouble() - mData[j] * (keduSpace * 1.0 / valueSpace)).toFloat(),
                    mPaintText
                )
            }
            //绘制柱状条
//            mPaintBar!!.setColor(colors[j])
            ...
            val r2 = RectF()
            r2.top = (startY - mData[j] * (keduSpace * 1.0 / valueSpace)).toFloat()
            r2.bottom = startY.toFloat()
            r2.left = initx.toFloat()
            r2.right = (initx + itemWidth).toFloat()

            val r3 = Rect()
            r3.top = startY - mData[j] * (keduSpace * 1 / valueSpace)
            r3.bottom = startY
            r3.left = initx
